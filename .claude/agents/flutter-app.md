...
Light:  배경 #FFF, 카드 #FAFAFA, 선 #EFEFEF, 텍스트 #262626/#8E8E8E
        포인트 #8B5CF6(violet-500), 하트 #ED4956, 스트릭 #7C3AED(violet-600)
...
- **성경 본문**: `height: 1.8` (leading-loose), 절번호 포인트(violet) bold
- **시스템 폰트 기본**, 한글 가독성 우선
...
| 8 | 한 줄 기도 | 하단 포인트 배경 카드 (violet-50 톤) |

### 3. 장 읽기 화면 — AI 설명 On-Demand

- 심플 `AppBar`: ← 뒤로 | 타이틀 | ♡ 하트
- 성경 본문: `ListView` 스크롤, `height: 1.8`, 절번호 포인트(violet)
- **하단 액션 바**: ♡ 좋아요 | 💡 설명 | 💬 공유
...
4. 캐시 미스 → GET /api/ai/explanation 호출
   → 스켈레톤 로딩 뷰 표시 (Shimmer 효과)
     - Container(borderRadius:16, color: violet50 또는 explanationCardBg)
...
│   │   └── theme.dart            # 테마 (라이트/다크, violet 포인트/스트릭)
│   ├── features/
│   │   ├── home/                 # 홈 탭 (8개 섹션 위젯)
│   │   ├── reader/               # 장 읽기
│   │   │   ├── chapter_reader.dart
